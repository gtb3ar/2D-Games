...
bool BoxCheckHit(float pointX, float pointY,float tlX, float tlY, float brX, float brY) {
    if (tlX <= pointX && tlY <= pointY && brX >= pointX && brY >= pointY) { //Check if point is inside the box
        return true;
    }
    else {
        return false;
